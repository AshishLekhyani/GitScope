...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const actionUrlValidation = body.actionUrl !== undefined
    ? validateAutomationActionUrl(rule.actionType, (body.actionUrl ?? "").trim() || null)
    : null;
  if (actionUrlValidation && !actionUrlValidation.ok) {
    return NextResponse.json({ error: actionUrlValidation.error }, { status: 400 });
  }
  ...
  const updated = await prisma.webhookRule.update({
    ...
    data: {
      ...
      ...(actionUrlValidation?.ok && { actionUrl: actionUrlValidation.url }),
