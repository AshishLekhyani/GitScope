  ...
  if (!isPro) {
    return (
      <div className="rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-8 text-center space-y-3">
        <MaterialIcon name="auto_awesome" size={32} className="text-indigo-400 mx-auto" />
    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-1 p-1 bg-surface-container/30 rounded-2xl border border-outline-variant/10">
        {GENERATORS.map((g) => (
          <button key={g.id} type="button"
            ...
            className={cn(
              "flex-1 flex items-center gap-2 px-4 py-2.5 rounded-xl transition-all",
              activeGen === g.id
            ...
            <div className="text-left">
              ...
              <p className={cn("text-[8px] mt-0.5 leading-tight hidden sm:block", activeGen === g.id ? "text-indigo-100/70" : "text-muted-foreground/40")}>{g.description}</p>
            </div>
          </button>
        ))}
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          {gen.options.map((opt) => (
            <div key={opt.key} className="space-y-1">
              ...
              <div className="flex gap-1 p-0.5 bg-surface-container/30 rounded-xl border border-outline-variant/10">
                {opt.values.map((v) => (
      ...
      {result && (
        <div className="space-y-3 animate-in fade-in duration-300">
          ...
          <pre className="whitespace-pre-wrap font-mono text-[11px] text-foreground/80 leading-relaxed bg-surface-container/30 rounded-2xl border border-outline-variant/10 p-5 overflow-x-auto max-h-130 overflow-y-auto">
            {result.text}
          </pre>
        </div>
      )}
